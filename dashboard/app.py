...
PAGES = {"overview": "📊 Overview", "explorer": "🔎 Explorer", "shap": "🧠 SHAP Explainer", "oracle": "🔮 Quantum Oracle", "nexus": "💎 Strategic Nexus", "threat": "🛡️ Threat Intelligence"}
...
""", unsafe_allow_html=True)
...
elif page_key == "threat":
    st.markdown('<div class="neon-title" style="font-size:2.2rem;">🛡️ THREAT INTELLIGENCE GRAPH</div>', unsafe_allow_html=True)
    st.markdown("<div class='neon-subtitle'>NETWORK ANALYSIS OF FRAUD RINGS & COMPROMISED IDENTITIES</div><br>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        try:
            ...
            pdf.cell(200, 15, txt="AEGISML CYBER COMMAND", ln=1, align='C')
            pdf.set_font("Arial", size=14, style='I')
            pdf.cell(200, 10, txt="Threat Intelligence Analytics Invoice", ln=1, align='C')
            pdf.ln(10)
            
            pdf.set_text_color(200, 200, 200)
            pdf.set_font("Arial", size=12)
            pdf.cell(200, 10, txt=f"Total Transactions Scanned: {len(df) if not df.empty else 0}", ln=1, align='L')
            if not df.empty and 'FraudProbability' in df.columns:
                pdf.cell(200, 10, txt=f"Critical Threats Blocked: {len(df[df['FraudProbability'] >= 0.75])}", ln=1, align='L')
                pdf.cell(200, 10, txt=f"Suspicious Anomalies: {len(df[(df['FraudProbability'] >= 0.4) & (df['FraudProbability'] < 0.75)])}", ln=1, align='L')
            
            pdf.ln(20)
            pdf.set_text_color(100, 100, 100)
            pdf.set_font("Arial", size=10, style='I')
            pdf.cell(200, 10, txt="Auto-generated by AegisML Enterprise SaaS", ln=1, align='C')
            
            pdf_bytes = pdf.output(dest='S').encode('latin1')
            st.download_button(label="📥 EXPORT PDF INVOICE", data=pdf_bytes, file_name="AegisML_Threat_Invoice.pdf", mime="application/pdf", use_container_width=True)
        except Exception as e:
            st.error(f"Failed to generate PDF: {e}")

    st.markdown('<div class="liquid-glass-card" style="margin-top:20px; min-height: 500px;">', unsafe_allow_html=True)
    if not df.empty and 'TransactionID' in df.columns:
        import networkx as nx
        
        # Subsample top 100 riskiest transactions for the graph
        if 'FraudProbability' in df.columns:
        ...
        for _, row in top_fraud.iterrows():
            txn_id = f"TXN-{row.get('TransactionID', 'Unknown')}"
            prob = row.get('FraudProbability', 0.5)
            G.add_node(txn_id, type="transaction", prob=prob)
            
            email = str(row.get('P_emaildomain', 'nan'))
            if email != 'nan' and email != 'unknown':
                G.add_node(email, type="email")
                G.add_edge(txn_id, email)
                
            device = str(row.get('DeviceInfo', 'nan'))
            if device != 'nan' and device != 'unknown':
        ...
        edge_trace = go.Scatter(x=edge_x, y=edge_y, line=dict(width=0.8, color='rgba(0,245,255,0.3)'), hoverinfo='none', mode='lines')
        ...
        for node in G.nodes():
            ...
            if ntype == "transaction":
                node_colors.append('#ff2d55') # Critical red
                node_texts.append(f"Transaction: {node}<br>Risk: {G.nodes[node].get('prob',0):.2f}")
                node_sizes.append(14)
            elif ntype == "email":
                node_colors.append('#00f5ff') # Cyan
                node_texts.append(f"Email Domain: {node}")
                node_sizes.append(10)
            else:
                node_colors.append('#b44fff') # Purple
                node_texts.append(f"Device: {node}")
                node_sizes.append(10)

        node_trace = go.Scatter(x=node_x, y=node_y, mode='markers', hoverinfo='text',
                                marker=dict(showscale=False, color=node_colors, size=node_sizes, line=dict(width=1, color='rgba(255,255,255,0.5)')))
        ...
        st.info("Awaiting CSV Payload to render Threat Graph.")
    st.markdown('</div>', unsafe_allow_html=True)
